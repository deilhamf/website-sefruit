...
@section('title', 'Heading Testimoni Data')
    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            ...
            <div class="card recent-sales overflow-auto">
                ...
                <div class="card-body">
                    <h5 class="card-title">@yield('title') <span>| Heading Testimonials Table</span></h5>
                    ...
                    <table class="table table-borderless datatable">
                        ...
                        <tbody>
                            @foreach ($headingtesti as $p)
                            <tr>
                                <th scope="row">{{ $loop->iteration }}</th>
                                <td>{!! $p->title !!}</td>
                                <td>{{ $p->subtitle }}</td>
                                <td><img src="{{ asset('heading/' . $p->image) }}" class="img-fluid"
                                        style="width: 90px; height: auto; border-radius: 10px;" alt="Gambar"></td>
                                <td>
                                    <div class="d-flex align-items-center gap-1">
                                        <a class="btn btn-primary btn-sm rounded-circle"
                                            href="{{ route('heading-testi.edit', $p->id) }}"><i
                                                class="bi bi-pencil-fill"></i></a>
                                        <form action="{{ url('/admin/heading-testi/' . $p->id) }}" method="post"
                                            class="d-inline">
                                            @csrf
                    ...
                    <form action="{{ url('/heading-testi/{id}') }}" method="POST" class="d-inline">
                        @csrf
                        @method('delete')
    ...
    function handleDelete(id)
        {
            ...
            link.href = "{{ URL::to('delete-heading-testi') }}/" + id
            $('#danger').modal('show')
        }
</script>
@endsection
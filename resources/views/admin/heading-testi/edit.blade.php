...
@section('title', 'Edit Heading Testi Data')
        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="row g-3" method="post" action="{{ route('heading-testi.update', $headingTesti->id) }}"
                        ...
                        <div class="col-md-6">
                            <label for="title" class="form-label"><i class="bi bi-chat-left-text-fill"></i>
                                Judul</label>
                            <input class="form-control" value="{{ $headingTesti->title }}" name="title"
                                required="required" placeholder="Masukkan title" id="title">
                            @error('title')
                        ...
                        <div class="col-md-6">
                            <label for="subtitle" class="form-label"><i class="bi bi-chat-left-text-fill"></i> Sub
                                Judul</label>
                            <input class="form-control" value="{{ $headingTesti->subtitle }}" name="subtitle"
                        ...
                        <div class="col-md-4">
                            <label for="image" class="form-label"><i class="bi bi-image-fill"></i> Upload
                                image</label>
                            <input type="file" class="form-control @error('image') is-invalid @enderror"
                                value="{{ old('image') }}" id="image" name="image" placeholder="image">
                            <img src="{{ asset('heading/' . $headingTesti->image) }}" class="img-preview img-fluid mt-2"
                                id="image-preview" width="90px" style="border-radius: 10px"> @error('image')
                            <div class="invalid invalid-feedback">{{$message}}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea type="text" id="task-textarea" class="form-control" id="desc" name="desc"
                                placeholder="Masukkan Deskripsi">{{ $headingTesti->desc }}
                                </textarea>
                            @error('desc')
                            <div class="invalid invalid-feedback">{{$message}}</div>
                            @enderror

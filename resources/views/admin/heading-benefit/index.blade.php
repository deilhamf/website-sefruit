...
@section('title', 'Heading Benfit Data')
    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            ...
            <div class="card recent-sales overflow-auto">
                ...
                <div class="card-body">
                    <h5 class="card-title">@yield('title') <span>| Heading Benefit Table</span></h5>
                    ...
                    <table class="table table-borderless datatable">
                        ...
                        <tbody>
                            @foreach ($headingbenefit as $p)
                            <tr>
                                <th scope="row">{{ $loop->iteration }}</th>
                                <td>{!! $p->title !!}</td>
                                <td>{{ $p->subtitle }}</td>
                                <td><img src="{{ asset('heading/benefit/' . $p->image) }}" class="img-fluid"
                                        style="width: 90px; height: auto; border-radius: 10px;" alt="Gambar"></td>
                                <td>
                                    <div class="d-flex align-items-center gap-1">
                                        <a class="btn btn-primary btn-sm rounded-circle"
                                            href="{{ route('heading-benefit.edit', $p->id) }}"><i
                                                class="bi bi-pencil-fill"></i></a>
                                        <form action="{{ url('/admin/heading-benefit/' . $p->id) }}" method="post"
                                            class="d-inline">
                                            @csrf
                    ...
                    <form action="{{ url('/heading-benefit/{id}') }}" method="POST" class="d-inline">
                        @csrf
                        @method('delete')
    ...
    function handleDelete(id)
        {
            ...
            link.href = "{{ URL::to('delete-heading-benefit') }}/" + id
            $('#danger').modal('show')
        }
</script>
@endsection
    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            ...
            <div class="card recent-sales overflow-auto">
                ...
                <div class="card-body">
                    <h5 class="card-title">@yield('title') <span>| About Us</span></h5>
                    ...
                    <table class="table table-borderless datatable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><img src="{{ asset('galeries/about/' . $p->image) }}" class="img-fluid"
                                        style="width: 90px; height: auto; border-radius: 10px;" alt="Gambar"></td>
                                <td>{{ $p->title }}</td>
                                <td>{{ $p->subtitle }}</td>
                                <td>{!! \Illuminate\Support\Str::limit($p->overview, 100) !!}</td>
                                {{-- <td>{!! $p->desc !!}</td> --}}
                                <td>
                                    <div class="d-flex align-items-center gap-1">
                                        <a class="btn btn-primary btn-sm rounded-circle"
                                            href="{{ url('/admin/about/' . $p->id . '/edit') }}">
                                            <i class="bi bi-pencil-fill"></i>
                                        </a>
                                        <form action="{{ url('/admin/about/' . $p->id) }}" method="post"
                                            class="d-inline">
                                            @csrf
                    ...
                    <form action="{{ url('/about/{id}') }}" method="POST" class="d-inline">
                        @csrf
                        @method('delete')
    ...
    function handleDelete(id)
        {
            ...
            link.href = "{{ URL::to('delete-about') }}/" + id
            $('#danger').modal('show')
        }
</script>
